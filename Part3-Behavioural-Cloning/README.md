...
The `model.py` file contains the code for training and saving the convolution neural network, although this file is mostly bits and pieces from the `Behavioural Cloning.ipynb`. The `Behavioural Cloning.ipynb` has all the code that I used to preprocess, train and evaluate the model, which is of more interest (and also more informative) than the `model.py` file
